...
app.post('/api/auth/resend-otp',async(req,res)=>{
  try{
    ...
    await user.save();
    await sendMail({to:email,subject:'Your new AgriConnect verification code',
...
app.post('/api/auth/login',async(req,res)=>{
  try{
    ...
    if(!user.isEmailVerified){
      ...
      user.emailVerifyOTP=otp; user.emailVerifyExpiry=new Date(Date.now()+15*60*1000);
      await user.save();
      await sendMail({to:user.email,subject:'Verify your AgriConnect account',
...
app.post('/api/auth/forgot-password',async(req,res)=>{
  try{
    ...
    await user.save();
    await sendMail({to:user.email,subject:'Reset your AgriConnect password',
...
app.delete('/api/products/:id',auth,async(req,res)=>{try{const p=await Product.findById(req.params.id);if(!p)return res.status(404).json({message:'Not found'});if(p.farmerID.toString()!==req.user.id&&req.user.role!=='Admin')return res.status(403).json({message:'Not authorised'});await p.deleteOne();
  // Sync trie
  productTrie.remove(req.params.id);
  res.json({message:'Deleted'});}catch(e){res.status(500).json({message:e.message});}});
